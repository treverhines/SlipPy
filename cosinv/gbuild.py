...
def flatten_vector_array(vec):  
  ''' 
  
  takes an array of vector values and returns a flattened array of 
  component values and the directions for each component
  
  The displacement array can be recovered by reshaping the component 
  values
  ...
  direction = np.zeros(vec.shape + (D,))
  for d in range(D): 
    direction[:,d,d] = 1.0

  direction = direction.reshape((-1,3))

  index = np.arange(N)[:,None].repeat(D,axis=1).flatten()
