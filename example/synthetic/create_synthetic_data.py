...
bm = cosinv.bm.create_default_basemap(pos_geo[:,0],pos_geo[:,1],resolution='i')
bm.drawcoastlines()
bm.drawstates()

seg_pos_cart = cosinv.bm.geodetic_to_cartesian(seg_pos_geo,bm)
pos_cart_f = cosinv.bm.geodetic_to_cartesian(pos_geo_f,bm)
pos_cart = cosinv.bm.geodetic_to_cartesian(pos_geo,bm)

### create synthetic slip
#####################################################################
P = cosinv.patch.Patch(seg_pos_cart,length,width,strike,dip)
...
cosinv.patch.draw_patches(Ps,colors=slip[:,0],edgecolor='none')

slip_basis = cosinv.basis.cardinal_basis((Ns,3))
...
#####################################################################
G = cosinv.gbuild.build_system_matrix(pos_cart_f,patches_f,disp_basis_f,slip_basis_f)
...
cosinv.quiver.quiver(pos_cart[:,0],pos_cart[:,1],disp[:,0],disp[:,1],
                     sigma=(sigma[:,0],sigma[:,1],0.0*sigma[:,0]),
                     scale=0.000001)

### write out synthetic data
#####################################################################
cosinv.io.write_gps_data(pos_geo,disp,sigma,'synthetic_gps.txt')
